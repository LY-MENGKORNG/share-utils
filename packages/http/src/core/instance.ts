...
export class Http {
  static create(input: Input, options: Options): ResponsePromise {
    ...
    const fun_ = async (): Promise<Response> => {
      if (
        typeof http.#options.timeout === "number" &&
        http.#options.timeout > maxSafeTimeout
      ) {
      ...
      if (http.#options.onDownloadProgress) {
        ...
        return streamResponse(
          progressResponse,
          http.#options.onDownloadProgress,
        );
      }
    ...
    for (const [type, mimeType] of Object.entries(
      ...
      if (
        type === "bytes" &&
        typeof (
          globalThis.Response?.prototype as unknown as { bytes?: unknown }
        )?.bytes !== "function"
      ) {
        continue;
      }
      ...
      result[type] = async () => {
        ...
        http.request.headers.set(
          "accept",
          http.request.headers.get("accept") || mimeType,
        );
  ...
  constructor(input: Input, options: Options = {}) {
    ...
    this.#options = {
      ...
      method: normalizeRequestMethod(
        options.method ?? (this.#input as Request).method ?? "GET",
      ),
      // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
    ...
    if (supportsAbortController && supportsAbortSignal) {
      this.#userProvidedAbortSignal =
        this.#options.signal ?? (this.#input as Request).signal;
    ...
    const userProvidedContentType =
      options.headers &&
      new globalThis.Headers(options.headers as HeadersInit).has(
        "content-type",
      );
    if (
  ...
  #clampRetryDelayToMax(retryDelay: number): number {
    ...
    return maxRetryAfter === undefined
      ? retryDelay
      : Math.min(maxRetryAfter, retryDelay);
  }
    ...
    try {
      return await Promise.race([
        Promise.resolve().then(() =>
          (this.#options.parseJson ?? JSON.parse)(text),
        ),
  ...
  async #fetch(): Promise<Response> {
    ...
    const remainingTimeout =
      this.#getRemainingTimeout() ?? this.#options.timeout;
    if (remainingTimeout <= 0) {
      throw new TimeoutError(this.request);
    }
  ...
  #getNormalizedOptions(): NormalizedOptions {
    if (!this.#cachedNormalizedOptions) {
      ...
      this.#cachedNormalizedOptions = Object.freeze(
        normalizedOptions,
