...
export class Http {
	static create(input: Input, options: Options): ResponsePromise {
		...
		const fun_ = async (): Promise<Response> => {
			...
			for (const hook of http.#options.hooks.afterResponse) {
				...
				const clonedResponse = http.#decorateResponse(response.clone());
			...
			if (
				...
				let error = new HTTPError(response, http.request, http.#getNormalizedOptions());
			...
			if (http.#options.onDownloadProgress) {
				...
				http.#cancelResponseBody(response);
				return streamResponse(progressResponse, http.#options.onDownloadProgress);
			}
		...
		if (hasSearchParameters(this.#options.searchParams)) {
			...
			const textSearchParams =
				typeof this.#options.searchParams === "string"
					...
					: new URLSearchParams(
							Http.#normalizeSearchParams(this.#options.searchParams) as unknown as SearchParamsInit
						).toString();
			// eslint-disable-next-line unicorn/prevent-abbreviations
	...
	async #retry<T extends (...arguments_: any) => Promise<any>>(
		function_: T
	): Promise<ReturnType<T> | Response | void> {
		try {
