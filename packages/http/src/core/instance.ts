import type { ObjectEntries } from "@repo/shared/types/mutation"
...
import { maxErrorResponseBodySize, responseTypes } from "#/constants/response"
import { ForceRetryError } from "#/errors/force-retry-error"
import { HTTPError } from "#/errors/http-error"
import { NonError } from "#/errors/non-error"
import { TimeoutError } from "#/errors/timeout-error"
import type { HttpRequest, HttpResponse, ResponsePromise } from "#/types"
import type {
	Input,
	InternalOptions,
	NormalizedOptions,
...
export class Http {
	static create(input: Input, options: Options): ResponsePromise {
		...
		const fun_ = async (): Promise<Response> => {
			if (typeof http.#options.timeout === "number" && http.#options.timeout > maxSafeTimeout) {
				throw new RangeError(`The \`timeout\` option cannot be greater than ${maxSafeTimeout}`)
			}

			// Track start time for total timeout across retries
			if (http.#startTime === undefined && typeof http.#options.timeout === "number") {
				http.#startTime = Date.now()
			}

			// Delay the fetch so that body method shortcuts can set the Accept header
			await Promise.resolve()
			// Before using http.request, _fetch clones it and saves the clone for future retries to use.
			...
			for (const hook of http.#options.hooks.afterResponse) {
				...
				const clonedResponse = http.#decorateResponse(response.clone() as Response) as HttpResponse

				const result = await safeTry.async(
					async () =>
						await hook({
							request: http.request as HttpRequest,
							options: http.#getNormalizedOptions(),
				...
				if (!result.success) {
					...
					http.#cancelResponseBody(clonedResponse)
					http.#cancelResponseBody(response)
					throw result.err
				}
				const modifiedResponse = result.value
			...
			http.#decorateResponse(response)

			if (
				!response.ok &&
				(typeof http.#options.throwHttpErrors === "function"
					? http.#options.throwHttpErrors(response.status)
					: http.#options.throwHttpErrors)
			) {
				let error = new HTTPError(
					response as HttpResponse,
					http.request as HttpRequest,
					http.#getNormalizedOptions()
				)
				error.data = await http.#getResponseData(response)
			...
			if (http.#options.onDownloadProgress) {
				if (typeof http.#options.onDownloadProgress !== "function") {
				...
				http.#cancelResponseBody(response)
				return streamResponse(progressResponse as Response, http.#options.onDownloadProgress)
			}
		...
		const result = http.#retry(fun_).finally(() => {
			const originalRequest = http.#originalRequest

			// Ignore cancellation errors from already-locked or already-consumed streams.
			http.#cancelBody(originalRequest?.body ?? undefined)
			http.#cancelBody(http.request.body ?? undefined)
		}) as ResponsePromise
		...
		>) {
			...
			if (
				type === "bytes" &&
				typeof (globalThis.Response?.prototype as unknown as { bytes?: unknown })?.bytes !==
					"function"
			) {
				continue
			}
			...
			result[type] = async () => {
				...
				http.request.headers.set("accept", http.request.headers.get("accept") || mimeType)

				const response = await result

				if (type === "json") {
					// Http intentionally returns `undefined` for 204/empty bodies on the shortcut for
	...
	constructor(input: Input, options: Options = {}) {
		...
		this.#options = {
			...
			retry: normalizeRetryOptions(options.retry) as InternalOptions["retry"],
		...
		if (this.#options.json !== undefined) {
			this.#options.body =
				this.#options.stringifyJson?.(this.#options.json) ?? JSON.stringify(this.#options.json)
		...
		const userProvidedContentType =
			options.headers && new globalThis.Headers(options.headers as HeadersInit).has("content-type")
		if (
		...
		this.request = new globalThis.Request(this.#input as Request, this.#options)

		if (hasSearchParameters(this.#options.searchParams)) {
			let textSearchParams: string
			if (typeof this.#options.searchParams === "string") {
				textSearchParams = this.#options.searchParams.replace(/^\?/, "")
			} else {
				this.#options.searchParams
				textSearchParams = new URLSearchParams(
					// @ts-expect-error
					Http.#normalizeSearchParams(this.#options.searchParams)
				).toString()
			}
	...
	#decorateResponse(response: Response): Response {
		if (this.#options.parseJson) {
			// biome-ignore lint/suspicious/noTsIgnore: <Reason: decode response>
			// @ts-ignore
	...
	async #readResponseText(response: Response, timeoutMs: number): Promise<string | undefined> {
		...
		const readerResult = safeTry(() => body.getReader())
		if (!readerResult.success) return undefined
	...
	// biome-ignore lint/suspicious/noExplicitAny: <Reason: Too flexible>
	async #retry<T extends (...arguments_: any) => Promise<any>>(
		function_: T
	): Promise<ReturnType<T> | Response | undefined> {
		try {
			return await function_()
		} catch (error) {
			const retryDelay = Math.min(await this.#calculateRetryDelay(error), maxSafeTimeout)
			const delayOptions = this.#userProvidedAbortSignal
			...
			if (remainingTimeoutAfterDelay !== undefined && remainingTimeoutAfterDelay <= 0) {
				throw new TimeoutError(this.request)
			}
			...
			for (const hook of this.#options.hooks.beforeRetry) {
				const hookResult = await hook({
					request: this.request as HttpRequest,
	...
	async #fetch(): Promise<Response> {
		...
		this.request = this.#originalRequest.clone() as HttpRequest
	...
	#getNormalizedOptions(): NormalizedOptions {
		if (!this.#cachedNormalizedOptions) {
			// Exclude Http-specific options that are not part of `RequestInit`.
			const {
				hooks: _hooks,
				json: _json,
				...
				timeout: _timeout,
				throwHttpErrors: _throwHttpErrors,
				fetch: _fetch,
				...normalizedOptions
			} = this.#options
