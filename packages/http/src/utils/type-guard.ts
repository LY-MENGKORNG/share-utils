...
export function isHTTPError<T = unknown>(error: unknown): error is HTTPError<T> {
	return error instanceof HTTPError || (error as HTTPError<T>)?.name === HTTPError.name
}
...
export function isTimeoutError(error: unknown): error is TimeoutError {
	return error instanceof TimeoutError || (error as TimeoutError)?.name === TimeoutError.name
}
...
export function isForceRetryError(error: unknown): error is ForceRetryError {
	return (
		error instanceof ForceRetryError || (error as ForceRetryError)?.name === ForceRetryError.name
	)
}
